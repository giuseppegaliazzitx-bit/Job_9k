...
const FIELD_GETTERS: Array<{ patterns: RegExp[]; get: Getter; knockout?: boolean }> = [
  ...
  { patterns: [/phone\s*device|device\s*type/i], get: (_p, a) => a.phone_device || "Mobile" },
  ...
  { patterns: [/graduation\s*year|year\s*of\s*graduation|class\s*of/i], get: (p) => p.education[0]?.end ?? "" },
  { patterns: [/\bgpa\b/i], get: (_p, a) => a.gpa ?? a.gpa_undergraduate ?? a["GPA (Undergraduate)"] ?? "" },
  { patterns: [/prior\s*worker|previously\s*worked|former\s*employee|employed.{0,24}in\s*the\s*past/i], get: (_p, a) => a.prior_worker || "No" },
